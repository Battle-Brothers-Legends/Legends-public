this.tactical_legend_tournament <- this.inherit("scripts/mapgen/tactical_template", {
	...
	function fill( _rect, _properties, _pass = 1 )
	{
		...
		local p =  patches[::Math.rand(0, patches.len() -1)];
		...
		local StandardPatch = this.MapGen.get(patch[::Math.rand(0, patch.len() -1)]);
		...
		local DryPatch = this.MapGen.get(patch[::Math.rand(0, patch.len() -1)]);
		...
		local WetPatch = this.MapGen.get(patch[::Math.rand(0, patch.len() -1)]);
		...
		local HillPatch = this.MapGen.get(patch[::Math.rand(0, patch.len() -1)]);
