this.legend_camp_legion_defend_cemetery_contract <- ::inherit("scripts/contracts/legend_camp_contract", {
	...
	function start() {
		if (this.m.Destination == null || this.m.Destination.isNull()) {
			...
			local locations = ::World.FactionManager.getFactionOfType(::Const.FactionType.Undead).getSettlements().filter(@(_, _loc) ::Legends.S.oneOf(_loc.getTypeID(), "location.undead_mass_grave", "location.undead_buried_castle"));
