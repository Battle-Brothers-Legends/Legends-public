this.legend_hunting_greenwood_schrats_action <- this.inherit("scripts/factions/faction_action", {
	...
	function onUpdate( _faction )
	{
		...
		if (_faction.getType()==::Const.FactionType.Settlement && !_faction.isReadyForContract(::Const.Contracts.ContractCategoryMap.legend_hunting_greenwood_schrats_contract))
		{
			return;
		}
		...
		local minResources = ::Const.World.LegendaryContract.GreenSchrat * ::Const.World.ContractCost.GreenSchrat + ::Const.World.ContractCost.GreenSchrat;

		local currentResources = this.getDifficultyMult() * this.getScaledDifficultyMult() * ::Const.World.ContractCost.GreenSchrat;
		...
		{
			::Const.World.LegendaryContract.GreenSchrat += 1;
		}
