this.legend_recruitment_nomad_encounter <- this.inherit("scripts/encounters/encounter", {
			...
			Options = [
				{
					...
					function getResult(_event) {
						::World.getPlayerRoster().add(_event.m.Nomad);
	...
	function isValid(_settlement) {
		if (::World.Assets.getOrigin().getID() != "scenario.legends_nomads")
			return false;

		local currentTile = ::World.State.getPlayer().getTile();
		if (currentTile.Type != ::Const.World.TerrainType.Desert) //must be desert
			return false;
	...
	function isVisible() {
		if (!::World.getTime().IsDaytime) //must be daytime
			return false;
		return true;
	}
