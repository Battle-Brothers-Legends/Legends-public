this.blackmarket_building <- this.inherit("scripts/entity/world/settlements/buildings/building", {
	...
	function isHidden() {
		return ::World.Assets.m.ProfessionEffect.LegendShadyDeals > 0 ? false : true;
	}
	...
	function getDefaultShopList() { //R = rarity (higher = more rare out of 1-99%) || P = price modifier (1.0 = normal price) || S = script of the item that will appear
		local item = @(r, p, s) {
			R = r,
			P = ::World.Assets.m.ProfessionEffect.LegendShadyDeals > 0 ? p / ::World.Assets.m.ProfessionEffect.LegendShadyDeals : p,
			S = s
		}
		local list = [];
		list.extend([
