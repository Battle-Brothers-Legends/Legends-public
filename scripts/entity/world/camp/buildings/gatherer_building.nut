this.gatherer_building <- this.inherit("scripts/entity/world/camp/camp_building", {
	...
	function getAllLevels()
	{
		...
		foreach( bro in roster )
		{
			...
			if (bro.getSkills().hasPerk(::Const.Perks.PerkDefs.LegendPotionBrewer))
			{
				map.Brewer += bro.getLevel();
			}

			if (bro.getSkills().hasPerk(::Const.Perks.PerkDefs.LegendSpecialistWoodaxeDamage))
			{
				map.Woodsman += bro.getLevel();
			}

			if (bro.getSkills().hasPerk(::Const.Perks.PerkDefs.LegendSpecialistPickaxeDamage))
			{
			...
			if (bro.getSkills().hasPerk(::Const.Perks.PerkDefs.LegendGatherer))
			{
				map.Apothecary += bro.getLevel();
			}
		}
