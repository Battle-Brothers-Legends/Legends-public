this.legend_unhold_throwing_net <- this.inherit("scripts/items/weapons/weapon", {
	...
	function create()
	{
		...
		this.m.Description = "A net provided by barbarian tribe, used to bind unholds. Tainted with webknecht poison, it makes stunned unholds docile creatures when trapped.";
	...
	function getTooltip()
	{
		...
		result.push({
			...
			text = "Will root a stunned unhold in place and make it docile"
		});
		result.push({
			id = 4,
			type = "text",
