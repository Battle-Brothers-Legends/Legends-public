this.legend_cured_rations_item <- this.inherit("scripts/items/supplies/food_item", {
	...
	function create() {
		...
		this.m.Description = "These rations have been hastily made for the purpose of survival, not taste. They will serve the same purpose but are unlikely to get much of a price at a market.";
	...
	function getSellPrice() {
		...
		if (("State" in ::World) && ::World.State != null && ::World.State.getCurrentTown() != null) {
			local isBuildingPresent = false;
			return ::Math.floor(this.getValue() * ::World.State.getCurrentTown().getFoodPriceMult() * ::World.State.getCurrentTown().getSellPriceMult() * (isBuildingPresent ? ::Const.World.Assets.BaseSellPrice : ::Const.World.Assets.SellPriceNotProducedHere));
		}
