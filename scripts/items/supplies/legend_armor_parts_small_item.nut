this.legend_armor_parts_small_item <- this.inherit("scripts/items/item", {
	...
	function getBuyPrice()
	{
		...
		{
			local isBuildingPresent = ::World.State.getCurrentTown().hasAttachedLocation("attached_location.workshop") || ::World.State.getCurrentTown().hasAttachedLocation("attached_location.leather_tanner") || ::World.State.getCurrentTown().hasBuilding("building.armorsmith") || ::World.State.getCurrentTown().hasBuilding("building.weaponsmith");
	...
	function consume()
	{
		::World.Assets.addArmorParts(this.m.Amount);
	}
