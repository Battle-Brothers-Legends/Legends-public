this.legend_transformed_effect <- this.inherit("scripts/skills/skill", {
	...
	function getTooltip()
	{
		...
		if (actor.getSkills().hasPerk(::Const.Perks.PerkDefs.LegendSurpressUrges))
		{
			ret.push({
				id = 11,
				type = "text",
	...
	function setAgents()
	{
		...
		if (actor.getSkills().hasPerk(::Const.Perks.PerkDefs.LegendControlInstincts))
		{
			return false;
		}
	...
	function removeItemsOnTransform()
	{
		...
		if (!this.m.Container.hasPerk(::Const.Perks.PerkDefs.LegendSurpressUrges))
		{
		...
		if (!this.m.Container.hasPerk(::Const.Perks.PerkDefs.LegendControlInstincts))
		{
		...
		if (this.m.Container.hasPerk(::Const.Perks.PerkDefs.LegendMasterAnger))
		{
