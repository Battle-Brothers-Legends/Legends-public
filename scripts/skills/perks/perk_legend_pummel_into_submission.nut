this.perk_legend_pummel_into_submission <- this.inherit("scripts/skills/skill", {
	...
	function onBeforeTargetHit( _skill, _targetEntity, _hitInfo )
	{
		...
		local fatDamage = ::Math.round(hitCopy.DamageFatigue * p.FatigueEffectMult * p.FatigueReceivedPerHitMult * p.FatigueLossOnAnyAttackMult);
		...
		if (overflow > 0)
		{
			...
			local damageRegular = (::Math.rand(properties.DamageRegularMin, properties.DamageRegularMax) + overflow) * properties.DamageRegularMult; // adding the overflow
			local damageArmor = (::Math.rand(properties.DamageRegularMin, properties.DamageRegularMax) + overflow) * properties.DamageArmorMult;
			damageRegular = ::Math.max(0, damageRegular + distanceToTarget * properties.DamageAdditionalWithEachTile);
			damageArmor = ::Math.max(0, damageArmor + distanceToTarget * properties.DamageAdditionalWithEachTile);
