this.legend_reserve_gets_fat_event <- this.inherit("scripts/events/event", {
	...
	function onUpdateScore() {
		...
		foreach (bro in brothers) {
			if (bro.isInReserves()) {
				if (bro.getFlags().get("IsSpecial") || bro.getFlags().get("IsPlayerCharacter") || ::Legends.Backgrounds.hasAny(bro, ::Legends.Background.LegendPuppet, ::Legends.Background.LegendDonkey)) {
					continue;
				}
