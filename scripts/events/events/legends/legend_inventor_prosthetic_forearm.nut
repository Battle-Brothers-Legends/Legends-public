this.legend_inventor_prosthetic_forearm <- this.inherit("scripts/events/event", {
	...
	function create()
	{
		...
		this.m.Cooldown = 45 * ::World.getTime().SecondsPerDay;
		this.m.Screens.push({
			ID = "A",
		...
		this.m.Screens.push({
			...
			function start( _event )
			{
				...
				::World.Assets.addArmorParts(-20);
				this.List.push({
					id = 10,
					icon = "ui/icons/asset_supplies.png",
					text = "You spend [color=" + ::Const.UI.Color.NegativeEventValue + "]20[/color] Tools and Supplies"
				});
	...
	function onUpdateScore()
	{
		...
		if (::World.Assets.getMoney() < 4000 || ::World.Assets.getArmorParts() < 40)
		{
			return;
		}
