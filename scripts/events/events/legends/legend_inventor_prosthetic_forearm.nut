this.legend_inventor_prosthetic_forearm <- this.inherit("scripts/events/event", {
	...
	function create()
	{
		...
		this.m.Screens.push({
			...
			function start( _event )
			{
				...
				local pros_foot_trait = this.new("scripts/skills/traits/legend_prosthetic_forearm");
	...
	function onUpdateScore()
	{
		...
		foreach (bro in brothers)
		{
			if (bro.getSkills().hasSkill("injury.broken_elbow_joint") && !bro.getSkills().hasSkill("trait.legend_prosthetic_forearm"))
			{
