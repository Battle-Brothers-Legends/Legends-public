this.legend_lose_fear_nobles_event <- this.inherit("scripts/events/event", {
	...
	function onUpdateScore() {
		...
		foreach (bro in brothers) {
			if (bro.getLifetimeStats().Battles < 25)
				continue;
			if (bro.getLifetimeStats().Kills < 50)
				continue;
			if (bro.getLifetimeStats().BattlesWithoutMe != 0)
				continue;
			if (!bro.getSkills().hasTrait(::Legends.Trait.LegendFearNobles))
				continue;
			candidates.push(bro);
		}
