this.legend_warhoundowner_event <- this.inherit("scripts/events/event", {
	...
	function onUpdateScore() {
		...
		if (::Stash.getNumberOfEmptySlots() < 3) {
			return;
		}

		local candidates_warhound_owner = [];
		foreach (bro in ::World.getPlayerRoster().getAll()) {
			...
			if (item != null && (item.getID() == "accessory.legend_warhound") || item != null && (item.getID() == "accessory.legend_armored_warhound") || item != null && (item.getID() == "accessory.legend_heavily_armored_warhound")) {
				candidates_warhound_owner.push(bro);
			}
		}
		if (candidates_warhound_owner.len() == 0) {
			return;
		}
		this.m.Houndhandler = candidates_warhound_owner[::Math.rand(0, candidates_warhound_owner.len() - 1)];

		local candidates_merchant = ::World.getPlayerRoster().getAll().filter(@(_, _bro) (::Legends.Backgrounds.has(_bro, ::Legends.Background.Peddler)));
		...
		this.m.Score = candidates_warhound_owner.len() * 10;
	}
