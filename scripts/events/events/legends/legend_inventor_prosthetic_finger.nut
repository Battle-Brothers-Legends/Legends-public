this.legend_inventor_prosthetic_finger <- this.inherit("scripts/events/event", {
	...
	function create()
	{
		...
		this.m.Screens.push({
			...
			function start( _event )
			{
				this.Characters.push(_event.m.Inventor.getImagePath());
				this.Characters.push(_event.m.Nofoot.getImagePath());
				::World.Assets.addMoney(-225);
				this.List.push({
					id = 10,
					icon = "ui/icons/asset_money.png",
					text = "You spend [color=" + ::Const.UI.Color.NegativeEventValue + "]225[/color] Crowns"
				});
	...
	function onUpdateScore()
	{
		...
		if (::World.Assets.getMoney() < 1000 || ::World.Assets.getArmorParts() < 10)
		{
			return;
		}
