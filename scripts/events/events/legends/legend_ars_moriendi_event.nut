this.legend_ars_moriendi_event <- this.inherit("scripts/events/event", {
	...
	function create() {
		...
		this.m.Screens.push({
			...
			function start(_event) {
				...
				foreach (bro in ::World.getPlayerRoster().getAll()) {
					if (::Legends.Traits.has(bro, ::Legends.Trait.Pessimist) && ::Math.rand(1, 100) <= 75) {
						entry = ::Legends.EventList.changeMood(bro, -1.0, "Didn't like the sermon about death");
						if (bro.getMoodState() < ::Const.MoodState.Neutral) {
							this.List.push(entry);
						}
					} else if (::Legends.Traits.has(bro, ::Legends.Trait.Optimist) && ::Math.rand(1, 100) <= 75) {
		...
		this.m.Screens.push({
			...
			function start(_event) {
				...
				foreach (bro in ::World.getPlayerRoster().getAll()) {
					if (::Legends.Traits.has(bro, ::Legends.Trait.Optimist) && ::Math.rand(1, 100) <= 75) {
						entry = ::Legends.EventList.changeMood(bro, 1.0, "Felt reassured by captain's words");
						if (bro.getMoodState() > ::Const.MoodState.Neutral) {
							this.List.push(entry);
						}
	...
	function onUpdateScore() {
		this.m.Score = 0;
		local brothers = ::World.getPlayerRoster().getAll();
		local candidates_scholar = brothers.filter(@(_, _bro)(::Legends.Backgrounds.hasAny(_bro, ::Legends.Background.Monk, ::Legends.Background.Historian) || ::Legends.Professions.has(_bro, ::Legends.Profession.LegendScholar)));
		...
		local self = this;
		local brothersWithoutScholar = brothers.filter(@(_, _bro)(_bro.getID() != self.m.Scholar.getID()));
		local candidates_cultist = brothersWithoutScholar.filter(@(_, _bro)(::Legends.Backgrounds.hasAny(_bro, ::Legends.Background.Cultist, ::Legends.Background.ConvertedCultist)));
		local candidates_deathwish = brothersWithoutScholar.filter(@(_, _bro)(::Legends.Traits.has(_bro, ::Legends.Trait.Deathwish)));
		local candidates_doubletongued = brothersWithoutScholar.filter(@(_, _bro)(::Legends.Traits.has(_bro, ::Legends.Trait.LegendDoubleTongued)));
		local candidates_optimist = brothersWithoutScholar.filter(@(_, _bro)(::Legends.Traits.has(_bro, ::Legends.Trait.Optimist)));
		local candidates_pessimist = brothersWithoutScholar.filter(@(_, _bro)(::Legends.Traits.has(_bro, ::Legends.Trait.Pessimist)));
