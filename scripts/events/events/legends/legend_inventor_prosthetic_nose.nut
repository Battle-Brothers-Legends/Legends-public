this.legend_inventor_prosthetic_nose <- this.inherit("scripts/events/event", {
	...
	function create()
	{
		...
		this.m.Cooldown = 35 * ::World.getTime().SecondsPerDay;
		this.m.Screens.push({
			ID = "A",
		...
		this.m.Screens.push({
			...
			function start( _event )
			{
				this.Characters.push(_event.m.Inventor.getImagePath());
				this.Characters.push(_event.m.Nofoot.getImagePath());
				::World.Assets.addMoney(-800);
				this.List.push({
					id = 10,
					icon = "ui/icons/asset_money.png",
					text = "You spend [color=" + ::Const.UI.Color.NegativeEventValue + "]800[/color] Crowns"
				});

				::World.Assets.addArmorParts(-10);
				this.List.push({
					id = 10,
					icon = "ui/icons/asset_supplies.png",
					text = "You spend [color=" + ::Const.UI.Color.NegativeEventValue + "]10[/color] Tools and Supplies"
				});
	...
	function onUpdateScore()
	{
		...
		if (::World.Assets.getMoney() < 1500 || ::World.Assets.getArmorParts() < 20)
		{
			return;
		}
