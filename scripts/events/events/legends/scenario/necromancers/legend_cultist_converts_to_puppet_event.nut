this.legend_cultist_converts_to_puppet_event <- this.inherit("scripts/events/event", {
	...
	function create() {
		...
		this.m.Screens.push({
			...
			Text = "[img]gfx/ui/events/event_39.png[/img]The proposal is too strange for you to consider, plus %cultist% does good work for %companyname% and the ritual would be dangerous. They remain disgruntled, but this will pass.",
			Image = "",
			List = [],
			Characters = [],
			Options = [
			...
			function start(_event) {
				this.Characters.push(_event.m.Cultist.getImagePath());
				this.List.push(::Legends.EventList.changeMood(_event.m.Cultist, 1.5, "Was rejected from ascending"));
			}
		});
	}
