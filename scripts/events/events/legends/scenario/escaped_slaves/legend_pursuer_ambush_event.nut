this.legend_pursuer_ambush_event <- this.inherit("scripts/events/event", {
	...
	function create() {
		...
		this.m.Screens.push({
			...
			function start(_event) {
				this.List.push(::Legends.EventList.changeMoney(-_event.m.BribeAmount));
			}
	...
	function onDetermineStartScreen() {
		...
		if (eventRoll >= 80 && canBeAssassins) return "Assassins";
		if (eventRoll >= 67 && canBeBountyHunters) return "BountyHunters";
		if (eventRoll >= 50 && canBeBribed) return "ManhuntersBribe";
		return "Manhunters";
	}
	...
	function onPrepare() {
		...
		if (this.m.DifficultyMult > 1.05) {
			this.m.BribeAmount = ::Math.round(::Math.rand(1200, 3000) * 0.01) * 100.0;
		} else if (this.m.DifficultyMult > 0.85) {
			this.m.BribeAmount = ::Math.round(::Math.rand(1000, 2500) * 0.01) * 100.0;
		} else {
