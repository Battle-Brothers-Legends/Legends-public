this.pursuer_ambush_event <- this.inherit("scripts/events/event", {
	m = {
		BribeAmount = 0.0,
		DifficultyMult = 0.0,
	...
	function create()
	{
		...
		this.m.ID = "event.pursuer_ambush";
			...
			Options = [
				{
					...
					{
						this.World.Statistics.getFlags().set("EscapedSlavesLastAmbushType", "Manhunters");
						...
						this.Const.World.Common.addUnitsToCombat(properties.Entities, this.Const.World.Spawn.EscapedSlavesManhunters, (100 + _event.m.ResourceBoost) * _event.m.DifficultyMult * _event.m.DifficultyMultScale, this.Const.Faction.Enemy, _event.m.ChampionChance);
			...
			Options = [
				{
					...
					function getResult( _event )
					{
						this.World.Statistics.getFlags().set("EscapedSlavesLastAmbushType", "ManhuntersBribe");
			...
			Options = [
				{
					...
					function getResult( _event )
					{
						...
						properties.PlayerDeploymentType = this.Const.Tactical.DeploymentType.Center;
						properties.EnemyDeploymentType = this.Const.Tactical.DeploymentType.Circle;
						this.Const.World.Common.addUnitsToCombat(properties.Entities, this.Const.World.Spawn.EscapedSlavesManhunters, (115 + _event.m.ResourceBoost) * _event.m.DifficultyMult * _event.m.DifficultyMultScale, this.Const.Faction.Enemy, _event.m.ChampionChance);
						this.World.State.startScriptedCombat(properties, false, false, true);
						return 0;
					}
			...
			Options = [
				{
					...
					function getResult( _event )
					{
						this.World.Statistics.getFlags().set("EscapedSlavesLastAmbushType", "Assassins");
						_event.m.NemesisCityState.addPlayerRelation(this.Const.World.Assets.RelationAttacked, "You evaded capture");
						local properties = this.World.State.getLocalCombatProperties(this.World.State.getPlayer().getPos());
						properties.CombatID = "Event";
			...
			Options = [
				{
					...
					function getResult( _event )
					{
						this.World.Statistics.getFlags().set("EscapedSlavesLastAmbushType", "BountyHunters");
						_event.m.NemesisCityState.addPlayerRelation(this.Const.World.Assets.RelationAttacked, "You evaded capture");
						local properties = this.World.State.getLocalCombatProperties(this.World.State.getPlayer().getPos());
						properties.CombatID = "Event";
						properties.Music = this.Const.Music.OrientalCityStateTracks;
	...
	function onDetermineStartScreen()
	{
		...
		local lastAmbush = this.World.Statistics.getFlags().get("EscapedSlavesLastAmbushType");
