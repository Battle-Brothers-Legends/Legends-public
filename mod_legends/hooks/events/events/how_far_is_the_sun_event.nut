::mods_hookExactClass("events/events/how_far_is_the_sun_event", function(o) {
	...
	o.onUpdateScore = function () {
		...
		foreach( bro in brothers ) {
			if (bro.getBackground().getID() == "background.historian" || bro.getBackground().getID() == "background.legend_inventor" || bro.getBackground().getID() == "background.legend_seer"  || bro.getBackground().getID() == "background.legend_seer_commander" || bro.getSkills().hasPerk(::Const.Perks.PerkDefs.LegendScholar))
