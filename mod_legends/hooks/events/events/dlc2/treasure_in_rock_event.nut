		...
		foreach (s in this.m.Screens) {
			if (s.ID == "Tiny") {
				...
				s.start <- function ( _event ) {
					...
					item = ::Const.World.Common.pickItem(weapons, "scripts/items/");
					::World.Assets.getStash().add(item);
					this.List.push({
						id = 10,
						icon = "ui/items/" + item.getIcon(),
					...
					if (_event.m.Tiny.getMoodState() >= ::Const.MoodState.Neutral) {
						this.List.push({
							id = 10,
							icon = ::Const.MoodStateIcon[_event.m.Tiny.getMoodState()],
							text = _event.m.Tiny.getName() + ::Const.MoodStateEvent[_event.m.Tiny.getMoodState()]
						});
					}
				}
			}
