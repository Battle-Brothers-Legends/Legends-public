	...
	o.onUpdateScore = function () {
		...
		foreach( bro in brothers ) {
			...
			if (bro.getBackground().isBackgroundType(::Const.BackgroundType.Combat))
				continue;
			if (bro.getPlaceInFormation() <= 17 && bro.getLifetimeStats().Battles >= 1)
				candidates.push(bro);
		}
