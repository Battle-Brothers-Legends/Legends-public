::mods_hookExactClass("events/events/houndmaster_tames_wolf_event", function (o) {
	...
	o.create = function () {
		...
		::Legends.Screens.hook(this, "A", function (_screen) {
			...
			_screen.start <- function (_event) {
				...
				this.List.push({
					id = 10,
					icon = ::Const.MoodStateIcon[_event.m.Houndmaster.getMoodState()],
					text = _event.m.Houndmaster.getName() + ::Const.MoodStateEvent[_event.m.Houndmaster.getMoodState()]
				});
			}
		});
	}
	...
	o.onUpdateScore = function () {
		...
		if (currentTile.Type != ::Const.World.TerrainType.Forest	&& currentTile.Type != ::Const.World.TerrainType.AutumnForest && currentTile.Type != ::Const.World.TerrainType.LeaveForest)
			return;
