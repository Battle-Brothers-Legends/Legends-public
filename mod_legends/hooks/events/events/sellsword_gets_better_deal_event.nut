::mods_hookExactClass("events/events/sellsword_gets_better_deal_event", function(o) {
	...
	o.create = function() {
		...
		::Legends.Screens.hook(this, "D", function(_screen) {
			_screen.Text = "[img]gfx/ui/events/event_04.png[/img] You sigh. %sellsword% nods and starts to leave, but you stop the mercenary. You\'ll pay the amount to stay the move. The %companyname% simply cannot afford to lose a blade like that.";
			_screen.Options[0].Text = "A good sword doesn\'t come cheap.";
		});
	}
	...
	o.onUpdateScore = function() {
		...
		foreach (bro in brothers) {
			...
			if (bro.getBackground().isBackgroundType(::Const.BackgroundType.ConvertedCultist))
				continue;
			if (bro.getFlags().has("convincedToStayWithCompany"))
				continue;
			if (::Legends.Backgrounds.has(bro, ::Legends.Background.Sellsword))
				candidates.push(bro);
		}
