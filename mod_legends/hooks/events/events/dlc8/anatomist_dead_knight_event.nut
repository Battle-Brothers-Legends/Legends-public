::mods_hookExactClass("events/events/dlc8/anatomist_dead_knight_event", function(o) {
		...
		foreach (s in this.m.Screens) {
			...
			if (s.ID == "C") {
				s.start <- function ( _event )
				{
					...
					local armor_list = [
						::Const.World.Common.pickArmor([[1, ::Legends.Armor.Standard.decayed_reinforced_mail_hauberk]]),
						::Const.World.Common.pickHelmet([[1, ::Legends.Helmet.Standard.decayed_closed_flat_top_with_mail]])
					];

					local item = this.new(armor_list[::Math.rand(0, armor_list.len() - 1)]);
					item.setCondition(item.getConditionMax() / 2 - 1);
					::World.Assets.getStash().add(item);
					this.List.push({
						id = 10,
	...
	o.onUpdateScore = function () {
		...
		foreach( bro in brothers ) {
			...
			else if (bro.getBackground().isBackgroundType(::Const.BackgroundType.Noble))
				noble_candidates.push(bro);
		}

		if (noble_candidates.len() > 0)
			this.m.Noble = noble_candidates[::Math.rand(0, noble_candidates.len() - 1)];
