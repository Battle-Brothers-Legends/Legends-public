::mods_hookExactClass("events/events/dlc8/disowned_noble_welcomed_back_event", function (o) {
		...
		foreach (s in this.m.Screens) {
			...
			if (s.ID == "C") {
				...
				s.start <- function (_event) //Uses King's guard format. Could use cultist onconvert function from character_background.nut but this would require a new savegame.
				{
					...
					_event.m.Disowned.getSkills().removeByID(::Legends.Backgrounds.getID(::Legends.Background.LegendDisownedNobleRanged));
	...
	o.onUpdateScore = function () {
		...
		foreach (bro in brothers) {
			if (::Legends.Backgrounds.hasAny(bro, ::Legends.Background.DisownedNoble, ::Legends.Background.LegendDisownedNobleRanged) && bro.getLevel() >= 6) {
