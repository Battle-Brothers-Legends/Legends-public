::mods_hookExactClass("skills/backgrounds/minstrel_background", function(o)
{
	o.create = function ()
	{
		...
		this.m.GoodEnding = "Ah, %name%. What an addition to the %companyname%! The minstrel not only became an excellent fighter, but was crucial in keeping the men\'s spirits high in the toughest of times. A poet and actor at heart, %they% eventually retired from the company and started up a theater company. %They% current performs plays for both the nobility and laymen. The minstrel does not yet realize it, but %their% playful wit and sharp commentary are slowly bringing the classes together.";
		this.m.BadEnding = "Never a fighter at heart, %name% the minstrel quickly left the declining %companyname%. %They% and a group of musicians and jesters spend their evenings performing for drunken noblemen. You managed to see one of these performances for yourself. %name% spent much of the time being berated by the inebriated and having half-eaten chicken bones thrown at %them%. One of the nobles even thought it\'d be funny to set a dog loose on one of the jesters. You could see the minstrel\'s dreams dying in %their% eyes, but the show went on.";
		this.m.Excluded = [
	...
	o.setGender <- function (_gender = -1) {
		...
		_gender ? this.setBodyCharacteristics(_gender, {HairColors = ::Const.HairColors.Young}) : this.setBodyCharacteristics(_gender, {Bodies = ::Const.Bodies.Skinny, Faces = ::Const.Faces.SmartMale, Hairs = ::Const.Hair.TidyMale, HairColors = ::Const.HairColors.Young, Beards = ::Const.Beards.Tidy});
