::mods_hookExactClass("skills/actives/coat_with_spider_poison_skill", function (o) {
	...
	o.getCurrentItem <- function () {
		// search for more poison bottles if the one under this.m.Item is empty
		if (this.m.Item.m.Ammo <= 0) {
			local self = this;
			local filledPoisons = this.getContainer().getActor().getItems().getAllItems().filter(@(_, _item)(_item.getID() == self.m.Item.getID()
				&& _item.m.Ammo > 0));
	...
	o.onUse = function (_user, _targetTile) {
		local poison = ::Legends.Effects.get(_user, ::Legends.Effect.SpiderPoisonCoat);

		if (poison != null) {
			poison.resetTime();
		} else {
			::Legends.Effects.grant(this.m.Container, ::Legends.Effect.SpiderPoisonCoat);
		}
