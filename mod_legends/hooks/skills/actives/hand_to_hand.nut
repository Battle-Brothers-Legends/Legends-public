::mods_hookExactClass("skills/actives/hand_to_hand", function(o)
{
	...
	o.getTooltip = function ()
	{
		...
		if (this.getContainer().getActor().getCurrentProperties().IsSpecializedInFists)
			fatigueMultiplier = 2;
		local fatPerHit = (this.getContainer().getActor().getCurrentProperties().FatigueDealtPerHitMult + fatigueMultiplier) * ::Const.Combat.FatigueReceivedPerHit;
		tooltip.push({
			id = 6,
			type = "text",
	...
	o.isUsable = function () // If ambidextrous & offhand free, or mainhand free, or disarmed, or net offhand
	{
		...
		if (::Legends.Perks.has(this, ::Legends.Perk.LegendAmbidextrous) && off == null && !items.hasBlockedSlot(::Const.ItemSlot.Offhand) && this.skill.isUsable)
			return true;
	...
	o.isHidden = function ()
	{
		...
		if (::Legends.Perks.has(this, ::Legends.Perk.LegendAmbidextrous) && off == null && !items.hasBlockedSlot(::Const.ItemSlot.Offhand)) // if ambidextrous && offhand free or with net, then NOT hidden
			return false;
