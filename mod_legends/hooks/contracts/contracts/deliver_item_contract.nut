::mods_hookExactClass("contracts/contracts/deliver_item_contract", function(o)
{
	...
	o.setup = function ()
	{
		...
		this.m.Payment.Pool = ::Math.max(75, distance * (4.0 + modrate) * this.getPaymentMult() * ::Math.pow(this.getDifficultyMult(), ::Const.World.Assets.ContractRewardPOW) * this.getReputationToPaymentLightMult());
		//	local modBonus = distance * modrate * this.getPaymentMult() * ::Math.pow(this.getDifficultyMult(), ::Const.World.Assets.ContractRewardPOW) * this.getReputationToPaymentLightMult();
				...
				s.Options = [
					{
						...
						function getResult()
						{
							...
							{
								::World.FactionManager.getFaction(this.Contract.getFaction()).addPlayerRelation(::Const.World.Assets.RelationNobleContractSuccess, "Delivered some cargo");
							}
							else
							{
				...
				s.Options = [
					{
						...
						function getResult()
						{
							...
							local playerRoster = ::World.getPlayerRoster().getAll();
							local xp = ::Math.round(::Const.Combat.GlobalXPMult * this.Contract.m.Payment.getOnCompletion() * 0.25);
							foreach( bro in playerRoster )
							{
								bro.addXP(xp);
								bro.updateLevel();
