::mods_hookExactClass("contracts/contracts/roaming_beasts_contract", function(o) {
	...
	o.onPrepareVariables = function ( _vars ) {
		...
		this.m.Helpful = candidates_helpful.len() != 0 ? candidates_helpful[::Math.rand(0, candidates_helpful.len() - 1)] : brothers[::Math.rand(0, brothers.len() - 1)];
		...
		local bro2 = candidates_bro2.len() != 0 ? candidates_bro2[::Math.rand(0, candidates_bro2.len() - 1)] : (brothers.len() > 1 ? brothersWithoutBro1[::Math.rand(0, brothersWithoutBro1.len() - 1)] : bro1);

		_vars.push([
			"helpfulbrother",
			this.m.Helpful.getName()
		]);
		_vars.push([
			"bro1",
			bro1.getName()
