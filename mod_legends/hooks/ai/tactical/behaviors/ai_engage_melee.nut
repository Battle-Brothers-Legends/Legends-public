		...
		else
		{
			...
			foreach( t in potentialDestinations )
			{
				...
				if (navigator.findPath(myTile, t.Tile, settings, acceptableDistanceFromDest))
				{
					...
					if (this.getProperties().PreferCarefulEngage && !::FU.IsAutoRetreat() && (hasAdrenaline || _entity.getTurnOrderInitiative() >= 125) && _entity.isAbleToWait() && !this.getStrategy().getStats().IsEngaged && !movementCosts.IsComplete && movementCosts.End.getDistanceTo(t.Tile) == 1 && movementCosts.Tiles > 1 && movementCosts.LastBeforeEnd.IsEmpty)
					{
