		...
		else if (this.Time.getVirtualTimeF() - this.m.Start >= this.m.Time)
		{
			...
			foreach( e in entities )
			{
				if (e.isAlive() && e.getID() == this.m.TargetID) {
					//Level
					if (e.getSize() == 1 && !e.isSouthern() && ::World.Assets.isPermanentDestruction()) {
						local news = ::World.Statistics.createNews();
						news.set("City", e.getName());
						::World.Statistics.addNews("crisis_greenskins_town_destroyed", news);
						::World.FactionManager.addGreaterEvilStrength(::Const.Factions.GreaterEvilStrengthOnTownDestroyed);
						e.setActive(false);
						e.getTile().spawnDetail(e.m.Sprite + "_ruins", ::Const.World.ZLevel.Object - 3, 0, false);
						e.fadeOutAndDie();
						::World.EntityManager.updateSettlementHeat();
					} else {
						local news = ::World.Statistics.createNews();
						news.set("City", e.getName());
						::World.Statistics.addNews("crisis_greenskins_town_razed", news);
						::World.FactionManager.addGreaterEvilStrength(::Const.Factions.GreaterEvilStrengthOnTownDestroyed);
