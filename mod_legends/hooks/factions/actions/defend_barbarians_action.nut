::mods_hookExactClass("factions/actions/defend_barbarians_action", function (o) {
	...
	o.onExecute = function (_faction) {
		...
		for (local i = 0; i < spawnpoints.len(); i++) {
			local party = this.getFaction().spawnEntity(spawnpoints[i], "Barbarians", false, ::Const.World.Spawn.Barbarians, this.m.Settlement.getResources() * 0.66 * ::Const.World.Scaling.getDistanceScaling(this, this.m.Settlement.getTile()));
