::mods_hookExactClass("factions/actions/defend_orcs_action", function (o) {
	...
	o.onExecute = function (_faction) {
		...
		for (local i = 0; i < spawnpoints.len(); i++) {
			local party = this.getFaction().spawnEntity(spawnpoints[i], "Orcs", false, ::Const.World.Spawn.OrcDefenders, this.m.Settlement.getResources() * 0.66 * ::Const.World.Scaling.getDistanceScaling(this, this.m.Settlement.getTile()));
