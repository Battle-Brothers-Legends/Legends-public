...
addNCSetting(config, ::MSU.Class.RangeSetting("FemaleGenderPercent", 25, 0, 100, 5, "Female Character %", "The higher the value, the more female characters will appear in the game. Default at 25%. Some characters are locked as male or female only and are not affected by this setting."));
addNCSetting(config, ::MSU.Class.SettingsDivider("ConfigDivider0")); // pushing the two settings below to one row to better communicate the settings' role to players
...
addNCSetting(config, ::MSU.Class.EnumSetting("CommanderAvatarGender", "Random", ["Random", "Male", "Female"], "Commander Avatar Gender", "Sets the gender of the commander brother in starts with an avatar present (Player Character Trait). Leave at random to have it follow the percentage specified in Female Character % setting."));
